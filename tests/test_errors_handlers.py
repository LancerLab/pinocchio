...
class TestRetry:
    ...
    def test_retry_with_specific_exceptions(self):
        ...
        mock_function = MagicMock(
            side_effect=[ValueError("Retry this"), TypeError("Don't retry this")]
        )
        mock_sleep = MagicMock()
    ...
    def test_retry_with_backoff(self):
        ...
        mock_function = MagicMock(
            side_effect=[ValueError("Attempt 1"), ValueError("Attempt 2"), "success"]
        )
        mock_sleep = MagicMock()
...
class TestCircuitBreaker:
    ...
    def test_circuit_breaker_half_open_state(self):
        ...
        breaker = CircuitBreaker(
            name="test-breaker", failure_threshold=2, recovery_timeout=0.1
        )
    ...
    def test_circuit_breaker_remains_open_after_half_open_failure(self):
        ...
        with pytest.raises(ValueError):
            breaker.execute(
                lambda: (_ for _ in ()).throw(ValueError("Failure in half-open"))
            )
