...
class TestPinocchioLogFormatter:
    ...
    def test_formatter_with_colors(self):
        ...
        # Check that color codes are included (test for presence of color codes, not exact format)
        assert "\033[91m" in formatted and "\033[0m" in formatted
    ...
    def test_formatter_with_error_context(self):
        ...
        # We're not truncating tracebacks in the formatter, so check for actual value
        assert "traceback: Long traceback text..." in formatted

    def test_formatter_custom_format(self):
        """Test PinocchioLogFormatter with custom format string."""
        formatter = PinocchioLogFormatter(
            fmt="%(levelname)s - %(message)s", datefmt="%H:%M:%S", use_colors=False
        )
...
class TestSetupLogging:
    ...
    def test_setup_logging_basic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("logging.StreamHandler") as mock_stream_handler, patch(
                ...
                assert (
                    mock_file_handler.call_count == 2
                )  # Regular log file and JSON log file
    ...
    def test_setup_logging_no_console(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("logging.StreamHandler") as mock_stream_handler, patch(
                ...
                setup_logging(
                    console_output=False, log_file=log_file, json_log_file=json_log_file
                )
    ...
    def test_setup_logging_no_file(self):
        ...
        with patch("logging.StreamHandler") as mock_stream_handler, patch(
            ...
            setup_logging(file_output=False, json_output=False)
